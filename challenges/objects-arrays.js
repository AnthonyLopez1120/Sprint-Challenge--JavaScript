...
let uniOnly = function(arr){
  let result = [];
  for (let i = 0; i< arr.length; i++){
   result.push(arr[i].university);
  }
  return result
}
universities = uniOnly(graduates)
...
lowCaseAnimalNames = zooAnimals.map(function(items){
...
lowPopulationAnimals = zooAnimals.filter(function(items){
